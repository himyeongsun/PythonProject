...
# 튜플 () : 여러개의 변수를 하나의 변수로 저장 (삽입,수정,삭제 불가)
# => 값의 위치를 첨자로 지정 [위치], (위치)
...
hap = aa[0] + aa[1] + aa[2] + aa[3] #리스트에 각 위치의 값을 더해서 합을 저장
print("합계=>%d"%hap)

# 리스트, 튜플, 딕셔너리는 값이 연속적으로 저장
# 선언 시 저장할 공간을 미리 확보
# 이미 확보된 공간을 수동으로 절대 추가, 삽입, 삭제 불가
